Vagrant.configure("2") do |config|
  Config::stable_version_list.each do |v|
    config.vm.provision "shell", privileged: false, inline: %[
      brew appimage-install -O -N appimage-#{Config::formula_name} -V #{v} -c #{Config::appimage_command} \
  ...
  if Config::devel_version_list[0] then
    ...
    config.vm.provision "shell", privileged: false, inline: %[
      brew appimage-install -O -N appimage-#{Config::formula_name} -V #{v2dev} -c #{Config::appimage_command} \
  ...
  config.vm.provision "shell", privileged: false, inline: %[
    brew appimage-install -O -N appimage-#{Config::formula_name} -V #{Config::head_version} -c #{Config::appimage_command} \
