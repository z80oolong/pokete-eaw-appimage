Vagrant.configure("2") do |config|
  ...
  if Config::stable_version? then
    config.vm.provision "shell", privileged: false, inline: "brew reinstall --without-python@3.9 #{Config::current_formula_name}@#{Config::current_version}"
  else
    config.vm.provision "shell", privileged: false, inline: %[
      brew reinstall --formula --without-python@3.9 #{Config::lib_dir}/#{Config::current_formula_name}@#{Config::current_head_formula_version}.rb
    ]
  end
end
