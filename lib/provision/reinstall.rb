Vagrant.configure("2") do |config|
  ...
  formula_versions.each do |v|
    config.vm.provision "shell", privileged: false, inline: "brew remove --force #{Config::formula_fullname}@#{v}"
    config.vm.provision "shell", privileged: false, inline: "brew install --without-python@3.9 #{Config::formula_fullname}@#{v}"
  end
  config.vm.provision "shell", privileged: false, inline: \
    "brew install --formula --without-python@3.9 #{Config::lib_dir}/#{Config::formula_name}@#{Config::head_version}.rb"
end
